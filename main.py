...
from agents.resilient_consensus_AC_agent import Resilient_CAC_agent
...
if __name__ == '__main__':
    ...
    parser.add_argument('--agent_label', help='classification of each agent (Cooperative,Malicious,Faulty)',type=str, default=['Cooperative','Cooperative','Cooperative','Cooperative'])
    ...
    parser.add_argument('--max_ep_len', help='Number of steps per episode', type=int, default=1)
    ...
    parser.add_argument('--n_ep_fixed',help='number of episodes under a fixed policy',type=int,default=1)
    parser.add_argument('--n_epochs',help='number of gradient steps in the critic and team reward updates',type=int,default=1)
    parser.add_argument('--in_nodes',help='specify a list of neighbors that transmit values to each agent (include the index of the agent as the first element)',type=int,default=[[0,1,2,3],[1,0,2,3],[2,0,1,3],[3,0,1,2]])
    ...
    parser.add_argument('--desired_state',help='desired state of the agents',type=int,default=np.random.randint(0,6,size=(4,2))) #np.array([[3,2],[3,4],[2,3],[1,4]]))
    ...
    args['desired_state'] = np.random.randint(0,6,size=(4,2))
    args['initial_state'] = np.random.randint(0,6,size=(4,2))
    ...
    for node in range(args['n_agents']):
        ...
        elif args['agent_label'][node] == 'Cooperative':
            print("This is a cooperative agent")                                          #create a cooperative agent
            agents.append(Resilient_CAC_agent(actor,critic,team_reward,slow_lr = args['slow_lr'],fast_lr = args['fast_lr'],gamma = args['gamma'],H = args['H']))
