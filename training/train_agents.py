...
'''
This file contains functions for training consensus AC agents in gym environments.
'''

def train_coop(env,agents,args):
    '''
    FUNCTION train_coop() - training a cooperative network
    ...
    using stochastic approximation and resilient consensus updates over multiple epochs. The updated parameters are then used
    in the evaluation of the actor gradient that is applied only once.
    ...
    for t in range(n_episodes):
        ...
        if i == n_ep_fixed-1:
            ...
            n,k = 1,1
            while n <= n_epochs:
                '''BATCH STOCHASTIC UPDATES OF THE CRITIC AND TEAM REWARD PARAMETERS'''
                ...
                for node in range(n_agents):
                    ...
                    if n == n_epochs:
                        TR_loss[node] += l1
                        critic_loss[node] += l2
                '''CONSENSUS UPDATES OF THE CRITIC AND TEAM REWARD PARAMETERS'''
                for node in range(n_agents):
                    critic_params_innodes = [critic_params[i] for i in in_nodes[node]]
                    TR_params_innodes = [TR_params[i] for i in in_nodes[node]]
                    agents[node].resilient_consensus(critic_params_innodes,TR_params_innodes)
                n = n+1
            '''BATCH ACTOR UPDATES'''
            for node in range(n_agents):
...
def train_mixed(env,agents,args):
    '''
    FUNCTION train_mixed() - training a mixed cooperative and adversarial network
    The agents are trained in batches using sampled actions from the actor network. In this realization, the agents accumulate local
    rewards from multiple episodes. At the end of the sequence of episodes, each agent updates for multiple epochs its critic and team
    reward parameters using stochastic approximation. All agents transmit their critic and team reward parameters but only cooperative
    agents perform resilient consensus updates. The updated parameters are used in the evaluation of the actor gradient that is applied
    only once. In addition to the critic and team reward updates, the adversarial agents separately update their local critic that is used
    in their actor updates.
    ...
    for t in range(n_episodes):
        ...
        states[i,j], rewards[i,j], done, _ = env.get_data()
        for node in range(n_agents):
            print(agents[node].critic.trainable_variables)
            estimated_ep_returns[node]=agents[node].critic(states[i,j].reshape(1,-1))
        ...
        if i == n_ep_fixed-1:
            ...
            while n <= n_epochs:
                ...
                critic_params,TR_params=[],[]
                for node in range(n_agents):
                    if args['agent_label'][node] == 'Malicious':
                        compromised_r = - np.mean(local_r,axis=1)           #Here goes the compromised reward - we take the negative average reward over all agents
                        x, l1 = agents[node].TR_update(s,team_a,compromised_r)
                        y, l2 = agents[node].critic_update(s,ns,local_r[:,node],compromised_r)
                    else:
                        x, l1 = agents[node].TR_update(s,team_a,local_r[:,node])
                        y, l2 = agents[node].critic_update(s,ns,local_r[:,node])
                    TR_params.append(x)
        ...
        with writer.as_default():
            ...
            tf.summary.scalar("true episode team-average rewards",np.mean(ep_rewards), step = t)
            writer.flush()
        print('| Episode: {} | Est. returns: {} | Returns: {} | Rewards: {} | Average critic loss: {} | Average TR loss: {} | Average actor loss: {} | Target reached: {} '.format(t,np.mean(estimated_ep_returns),np.mean(ep_returns),np.mean(ep_rewards),critic_loss,TR_loss,actor_loss,done))
        path = {
                "Episode_rewards":np.array(ep_rewards),
                "True_team_returns":np.mean(ep_returns),
                "Estimated_team_returns":np.mean(estimated_ep_returns)
...
def train_online(env,agents,args):
    '''
    FUNCTION train_mixed() - training a mixed cooperative and adversarial network
    The agents are trained online using sampled actions from the actor network. In this realization, the agents apply the latest rewards.
    ----At the end of the sequence of episodes, each agent updates for multiple epochs its critic and team
    reward parameters using stochastic approximation. All agents transmit their critic and team reward parameters but only cooperative
    agents perform resilient consensus updates. The updated parameters are used in the evaluation of the actor gradient that is applied
    only once. In addition to the critic and team reward updates, the adversarial agents separately update their local critic that is used
    in their actor updates.
    ...
    for t in range(n_episodes):
        ...
        while j < max_ep_len:
            ...
            #-------------------------------------------------------------------
            '''AGENT TRAINING'''
            s = states[i,j].reshape(1,-1)
            ns = states[i,j+1].reshape(1,-1)
            local_r = rewards[i,j].reshape(1,n_agents,1)
            local_a = actions[i,j].reshape(1,n_agents,1)
            team_a = actions[i,j].reshape(1,n_agents)
            n = 1
            '''STOCHASTIC UPDATE OF THE CRITIC AND TEAM REWARD PARAMETERS'''
            critic_params,TR_params=[],[]
            for node in range(n_agents):
                if args['agent_label'][node] == 'Malicious':
                    compromised_r = - np.mean(local_r,axis=1)           #Here goes the compromised reward - we take the negative average reward over all agents
                ...
                TR_params.append(x)
                critic_params.append(y)
                TR_loss[node] += l1/max_ep_len
                critic_loss[node] += l2/max_ep_len
            '''CONSENSUS UPDATES OF THE CRITIC AND TEAM REWARD PARAMETERS'''
            for node in range(n_agents):
                if args['agent_label'][node] == 'Cooperative':
                    critic_params_innodes = [critic_params[i] for i in in_nodes[node]]
        ...
        with writer.as_default():
            ...
            tf.summary.scalar("true episode team-average rewards",np.mean(ep_rewards), step = t)
            writer.flush()
        print('| Episode: {} | Est. returns: {} | Returns: {} | Rewards: {} | Average critic loss: {} | Average TR loss: {} | Average actor loss: {} | Target reached: {} '.format(t,estimated_ep_returns,ep_returns,np.mean(ep_rewards),critic_loss,TR_loss,actor_loss,done))
        path = {
                "Episode_rewards":np.array(ep_rewards),
                "True_team_returns":np.mean(ep_returns),
                "Estimated_team_returns":np.mean(estimated_ep_returns)
